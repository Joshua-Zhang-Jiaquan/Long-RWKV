...
assertion until it was run. Twenty-five substantive numbers this manuscript cites are
listed in \texttt{DAN/nonlatent\_iclr/paper\_claims.json} against the artifact that should
carry each one and the \emph{context} it must appear in, and
\texttt{scale/experiments/nonlatent\_iclr/paper\_evidence\_check.py} checks all of them
deterministically, with no model in the loop. It returns five verdicts, and the
