...
\paragraph{The controlled ablation, running and partial.} The arm matrix of
\S\ref{sec:protocol} is in flight, and one of its four arms has completed all three seeds.
Reporting it now is worth doing, because the number that has arrived is not a result about
the architecture --- it is the \emph{yardstick} every result will be judged against.

Arm A1 (forward-only, no recycling) finished its frozen $2$B-token budget at
$\mathrm{mask\_ce}$ of $5.6091$, $5.1118$ and $5.6413$ on seeds $17$, $29$ and $43$: a mean
of $5.4541$ and a \textbf{seed spread of $0.5295$}. The analysis reports a contrast as
separable only when it exceeds the larger of the two arms' spreads, so any between-arm
difference smaller than about $0.53$ in this metric is not a finding. We state the yardstick
before the contrast deliberately: with three seeds, a difference inside the seed spread is
indistinguishable from noise, and the pre-registered interpretation of an arm that does not
clear it is that recycling bought no detectable quality at this scale and budget --- which
is a result, not a failure.

Against that bar, the contrasts the protocol declares --- directionality (A2 against A1),
the loop on a bidirectional backbone (A3 against A2), and the loop alone (A5 against A1)
--- are all \notrun{} and will remain so until the remaining nine runs finish. The
comparison is made on the runs' own training-time validation metric, not on a benchmark: a
separable difference here would justify a benchmark evaluation, and its absence would not by
itself rule the mechanism out at larger scale, which would need its own budget. Both
readings are pre-registered.
