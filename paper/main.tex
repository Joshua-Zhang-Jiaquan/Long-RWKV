...
A recurrent language model carries a fixed-size state forward and reads the next token
from it. That state is the only channel through which context reaches the present, and it
is also the bottleneck: whatever does not fit is gone. The promise is a cost that does not
grow with context; the price is that the same object must serve as the memory. Attention
sidesteps the bottleneck by keeping every token addressable and paying for it with a cache
that grows linearly, which is why a served long-context system is usually measured in cache
gigabytes rather than in parameters.

This paper studies a model that tries to take the recurrent bargain seriously. It is a
\emph{non-latent} recurrent diffusion model: a \textbf{bidirectionally scanned RWKV-7
backbone that denoises masked tokens in place}, with \textbf{no learned latent sidecar} and
\textbf{no separate decoder}. The object under study is concrete and fixed. The backbone is
RWKV-7 with $4{,}091{,}581{,}441$ parameters, hidden width $2{,}560$, $32$ layers, $40$
heads of dimension $64$, and a vocabulary of $65{,}536$; it is scanned once forward and once
backward and the two scans are fused by a learned gate. Denoising happens on the token
canvas itself: masked positions are replaced by predictions, and the process repeats,
so the model can revise a token rather than commit to it. There is no sidecar latent to
carry meaning between steps and no separate decoder to render it --- the state, the canvas,
and the output are all the same object.

We call this \emph{non-latent} to make the contrast explicit, because the latent variant is
the default in the diffusion-language-model literature and it changes what is being
measured. A model that denoises \emph{in place} must represent its partial answer in the
same tokens it will emit, so every intermediate state is directly inspectable and directly
scorable. A model with a learned latent sidecar can accumulate information that its decoder
never emits, which is a capability, and also a confound: a long-context gain may come from
the sidecar rather than from the recurrent state, and no measurement of the emitted tokens
can tell the difference. Our object has no such place to hide, which is what makes the
questions below well posed --- and it is a deliberate restriction we pay for, not a
capability we claim.

Against that object we ask about three levers, in increasing order of how cheaply they
might buy depth:

\begin{itemize}
  \item \textbf{Direction.} A causal scan lets a token see only its past. A bidirectional
  scan lets every position see left \emph{and} right context within the window, at equal
  parameter count and without a cache.
  \item \textbf{Denoising.} An autoregressive model commits to each token as it emits it.
  An iterative denoiser can withdraw a decision, which is exactly what a token whose
  meaning depends on a later one needs.
  \item \textbf{Depth recycling.} Re-applying a \emph{tied} block more times than the
  backbone has layers adds executed depth without adding parameters. It is the cheapest of
  the three, because the cost is compute rather than memory or storage, and it is the one
  this paper is organised around.
\end{itemize}

The paper's central question is whether that third lever does anything. It is organised
around the possibility that the answer is \emph{no}, and we state the two failure modes in
advance because both are unremarkable. Recycling may be an expensive way to spend compute
that a wider baseline would have spent better --- extra depth that is merely extra, wearing
an architectural story. And any short-context benefit may simply not transfer to long
context, where a fixed-size state rather than executed depth is the binding constraint. We
therefore commit to the comparisons that would distinguish a real architectural advantage
from those two, and to the interpretation of a null result, before running them.

\paragraph{What we can already measure, and what it forbids us from claiming.} One part of
the architecture's cost is measurable today, without any training campaign, and its answer
is not the one a casual reading of ``linear attention is cheaper'' would predict. We measure
peak device memory and throughput for the model under study and for four other checkpoints
across four context lengths, on one H100, with an untimed warm-up and \texttt{inference\_mode}
(\S\ref{sec:efficiency}). The recurrent arm's state is constant --- $20.0$\,MiB at every
context, against a same-size transformer's key--value cache of $7{,}168$\,MiB at $64$K ---
and its per-forward throughput is flat across context where the transformers roughly halve.
Its \emph{answer}-level throughput, however, is about $175\times$ lower, because a
masked-diffusion answer costs $100$ forward passes against an autoregressive model's one.
The honest reading is a \textbf{trade}: a structurally smaller state and far flatter
degradation with context, paid for in forward passes per answer. Any sentence claiming this
model is cheaper in \emph{compute} is one this paper will not write, and we report the two
readings side by side precisely so that the flattering one cannot be quoted alone.

\paragraph{Corrections, treated as a contribution rather than a preface.} A substantial part
of this paper is about the program's own earlier evidence, because several of its prior
conclusions do not survive re-analysis. A generation path truncated at the wrong
end-of-text id silently zeroed an entire evaluation --- \emph{exactly} $0.0$ on $164$ of
$164$ tasks across three successive training-data revisions, a number whose invariance
across corpus changes was itself the tell. An internal record carried an unresolved
tokenizer-id conflict that had to be settled from the vocabulary rather than by choice. A
GSM8K score turned out to measure an answer extractor rather than a model: the prompt asks
for the answer in \texttt{\textbackslash boxed\{\}}, the requested extraction tier fired on
$0$ and $1$ items of $200$ across the two cells that stored their text, and every success
came from a last-number fallback over degenerate completions. These share one shape --- a
plausible constant, never checked against the artifact that defines it, in a position where
being wrong is silent --- and we document the class rather than the three instances,
including two further instances the same audit found in \emph{this} manuscript
(\S\ref{sec:correction}).

\paragraph{Why this manuscript looks the way it does.} The program has completed its
evidence audit, its architecture contract, its asset qualification and its runtime
calibration, and has \emph{not} completed the confirmatory training and evaluation campaign.
We have written the paper to that state: the completed work is reported as results, the
protocol is reported as a registrable protocol, and the cells the protocol defines are left
visibly empty and \emph{named} as empty. Two claims are in a different condition from
``not yet measured'' and we distinguish them throughout: the long-context, serving and
depth-recycling claims describe work this program intends and has not finished, while the
physical edge-device claim is out of scope permanently, because a software memory cap on an
H100 cannot become a statement about edge silicon. We think a paper written to this state is
more useful than either a premature results paper or a plan, and the alternative --- filling
the empty cells with fixture numbers --- is excluded by the program's own rules.
...
scored a checkpoint at $4/200$ and, at a second decode setting, $5/200$ --- and
neither score is a measurement of arithmetic ability. The evaluation's prompt asks
explicitly for the answer in \texttt{\textbackslash boxed\{\}}, and classifying all
two hundred completions \textbf{by the scorer's own extraction tiers} gives, for the
two cells that stored their generated text: the boxed tier fired on $0$ and $1$ items
respectively, the last-number fallback on $117$ and $126$ ($4$ and all $5$ of the
successes), a last-line fallback on $76$ and $71$ ($0$ correct), and the remainder
carried no text. So \emph{none} of the nine successes came from the tier the prompt
requests; every one came from a last-number rule reading a number out of generative
noise, on both cells independently. That the score is noise rather than ability is
testable, not merely arguable: under the null that the extractor draws uniformly from
the numbers each completion happens to emit --- \emph{scoped to the items the fallback
tier scored}, since the null's claim is only that the fallback matched by chance ---
the expected counts are $3.60$ and $4.77$ against observed $4$ and $5$, with
$P(X \geq k) = 0.52$ and $0.56$. Both are inseparable. We state the null in its unscoped form --- every completion that emits a number --- which on this data is the more conservative of the two available definitions and needs no reference to a campaign-specific gating decision. A gate needs the scoped form instead, which excludes items the requested tier scored, because a genuine worked solution repeats its answer and an unscoped null therefore penalises the very behaviour it is meant to certify. The completions are degenerate
rather than merely wrong, with $33\%$ spending at least three quarters of their tokens
on one repeated word and a longest identical run of $511$. The other cells of that
grid stored scores but not text, so they are \notrun{} for this question rather than
corroborating it --- a distinction we make explicitly, because a grid of agreeing
percentages would otherwise read as confirmation of a claim none of them tested. The
figure is reported nowhere in this paper --- our descriptive measurements are
OpenBookQA and RACE --- and we record it because the lesson is the same one the
decode-side stop set taught: \emph{a number can be perfectly reproducible and still not
be about the thing its label names}. A detail worth keeping: the first statement of
this finding counted whether the \texttt{\textbackslash boxed} \emph{substring}
appeared. It appears, once, in a completion that emits the token with no braces, which
the scorer's brace-balanced parser therefore rejects --- so the substring was present
and the tier does NOT fire; on the second the group parses, the tier fires, and the answer is simply wrong. Across the four hundred completions of the two text-bearing cells the requested tier therefore fired exactly once and scored zero, which is the sentence that matters and is stronger than either ``it never fires'' or ``it usually fires''. Counting a substring is not counting what the code does --- and neither is counting how often the code runs, without asking what it found.
...
\section{Efficiency, measured}
\label{sec:efficiency}

This is the part of the paper's cost story that does not wait for the training campaign. It
is a measurement of the architecture as it exists, on one H100, with every number taken in
\texttt{torch.inference\_mode}, one untimed warm-up forward per rung and five timed ones, and
peak device memory read from \texttt{reset\_peak\_memory\_stats}. It is \emph{not} the
existing training-shape calibration, which builds an autograd graph and calls backward and
therefore overstates inference cost; that calibration also exhausted memory at $4{,}096$
tokens for the recycled arm, which is why an inference-only ladder was necessary before any
claim about serving could be made.

Four of the six registered checkpoints load in this environment. Two do not, and they are
reported as holes rather than omitted (\S\ref{sec:efficiency-holes}).

\begin{table}[t]
\centering
\begin{tabular}{lrrrrr}
\toprule
model & family & objective & NFE & params & state at $64$K \\
\midrule
the model under study & linear attention & masked diffusion & $100$ & $4,092$M & $20.0$\,MiB \emph{constant} \\
RWKV7-Goose-World3   & linear attention & autoregressive   & $1$   & $2,948$M & $10.0$\,MiB \emph{constant} \\
Qwen2.5-3B           & transformer      & autoregressive   & $1$   & $3,086$M & $2{,}304$\,MiB \\
Llama-3.2-3B         & transformer      & autoregressive   & $1$   & $3,213$M & $7{,}168$\,MiB \\
\bottomrule
\end{tabular}
\caption{The four checkpoints that load. \emph{State} is arithmetic from the configuration,
not a measurement --- the adapter that computes it says so --- and it is the one column here
that does not depend on the run. A same-size transformer's key--value cache grows linearly
and reaches $7{,}168$\,MiB at $64$K where the recurrent state is $20.0$\,MiB at every
length: a factor of $358$. Grouped-query attention would shrink the transformer side and is
not modelled.}
\label{tab:eff-state}
\end{table}
...
\toprule
 & $4$K & $16$K & $32$K & $64$K \\
\midrule
\multicolumn{5}{l}{\emph{per-forward throughput}, tok/s} \\
the model under study & 16,990 & 17,716 & 17,810 & 18,017 \\
RWKV7-Goose-World3   & 26,405 & 27,969 & 28,061 & 28,477 \\
Qwen2.5-3B           & 68,445 & 58,753 & 48,539 & 35,156 \\
Llama-3.2-3B         & 65,331 & 53,068 & 43,543 & 31,995 \\
\midrule
\multicolumn{5}{l}{\emph{answer} throughput, tok/s, i.e. per-forward $\div$ NFE} \\
the model under study & 16,990 & 17,716 & 17,810 & 18,017 \\
\bottomrule
\end{tabular}
\caption{Measured on one H100 with an untimed warm-up, five timed forwards per cell, peak
from \texttt{reset\_peak\_memory\_stats}. The two panels are the SAME measurements read at
two units of work, and they are printed together because either alone misleads: the
per-forward row is flat across context where the transformers roughly halve, and the answer
row is $175\times$ lower because a masked-diffusion answer costs $100$ forwards against an
autoregressive model's one. Values are the artifact's, rounded to the nearest integer --
earlier drafts of this table rounded to round numbers, which turned $68{,}444.6$ into
$68{,}400$ and $27{,}968.9$ into $28{,}000$: a display choice that silently misstates the
measurement.}
\label{tab:eff-measured}
\end{table}
...
\begin{tabular}{lrrrr}
\toprule
peak allocated memory, GiB & $4$K & $16$K & $32$K & $64$K \\
\midrule
the model under study & $8.49$ & $11.01$ & $14.37$ & $21.09$ \\
RWKV7-Goose-World3   & $12.53$ & $16.67$ & $22.17$ & $33.19$ \\
Qwen2.5-3B           & $7.20$ & $11.15$ & $16.41$ & $26.93$ \\
Llama-3.2-3B         & $7.46$ & $11.77$ & $17.53$ & $29.05$ \\
\bottomrule\end{tabular}
\caption{Peak allocated device memory, same runs. The margin between the recurrent arms and
the transformers widens with context, which is the shape a constant state predicts: the
model under study grows $2.48\times$ from $4$K to $64$K where Llama grows $3.90\times$.}
\label{tab:eff-peak}
\end{table}

\paragraph{What the measurement supports.} Two claims, and neither more.

First, \emph{the state is structurally smaller and does not grow with context}. In the
current implementation the recurrent state is $20.0$\,MiB at every length tested, against a
same-size transformer's $7{,}168$\,MiB at $64$K --- a factor of $358$, from arithmetic on
the configuration rather than a measurement. The measured peak memory corroborates the
direction at the margin: the model under study grows $2.48\times$ from $4$K to $64$K where
Llama grows $3.90\times$ and Qwen $3.74\times$, and the residual growth is activations and
the canvas rather than the state.

Second, \emph{per-forward throughput is flat across context} for the recurrent arms
($16{,}990 \rightarrow 18{,}017$) where the transformers fall by about half
($65,331 \rightarrow 31{,}995$ for Llama). This is the recurrence doing what it is
supposed to do: no cache to grow means no cache to walk.

\paragraph{What the timing method does not support.} The throughput figures are a
\emph{short-burst} measurement: one untimed warm-up and five timed forwards per cell, taken
over seconds. They are not a sustained-load measurement, and the two differ for exactly the
workload this section is about. Long-context inference is memory-bandwidth bound, and a
sustained run can see clocks settle and bandwidth utilisation saturate in ways a five-forward
average cannot show. A gate that certifies a submission on a sustained device-memory
bandwidth median --- a shape this program has had proposed internally, requiring on the order
of a hundred rows per rank to reach a $90\%$ median --- is measuring a quantity this
experiment does not produce, and we do not present our numbers as if it did.

What this means for the claims above: the \emph{rank ordering} and the \emph{growth with
context} are robust to the caveat, because every model was measured the same way and the
comparison is between arms rather than against an absolute ceiling. The absolute tokens per
second are a burst rate. A serving claim would need a sustained measurement at a declared
load, which is part of the registered deployment protocol and remains \notrun{}.

\paragraph{What the measurement forbids.} A compute claim. At $64$K the model under study
produces $180$ tokens per second of wall clock against Llama's $31{,}995$ and Qwen's
$35{,}156$, because producing one answer requires $100$ forward passes. That is roughly a
$175\times$ deficit, and it is not an implementation artifact: the iterative refinement
\emph{is} the model. A paper claiming this architecture is cheaper to \emph{serve} would be
claiming something its own measurements contradict.

What remains open, and is the honest form of the claim: whether the state advantage can be
\emph{exploited}. A constant state is only cheap if a served system can keep it and skip the
recomputation a cache would have avoided; this program's prefix-cache path is
\textsc{not yet measured} (the model's forward-stream state is unwired through the FFN
token-shift), so the $358\times$ figure is a statement about a memory \emph{footprint}, not
about a served latency or a dollar cost. We state it as the former.

\paragraph{The two holes.}
\label{sec:efficiency-holes}
Two of the six registered checkpoints could not be loaded in the measured environment and
appear as \texttt{load\_failed} rows with their error text, not as omissions:
\emph{LLaDA-8B-Base}, whose remote modelling code expects a \texttt{transformers} attribute
that version $5.3$ no longer provides, and \emph{mamba2-2.7b}, whose configuration carries
no \texttt{model\_type} and therefore needs its own loader. Both are recorded in the
artifact with the reason. We would rather print a table with two named holes than a complete
table whose rows were produced by shimming a loader whose correctness we could not check.

What this does \emph{not} yet have is the paired comparison the protocol registers: the
serving measurement at a common SLO, and the long-context evaluation. Those remain
\notrun{}, and \S\ref{sec:not-measured} names them.
...
\paragraph{The controlled ablation: one lever engaged, one did not.} All twelve runs
reached the frozen $2$B-token budget, and the result is not the one the design anticipated.
The four arms, on seeds $17$/$29$/$43$, at $\mathrm{mask\_ce}$:

\begin{center}
\begin{tabular}{llrrr}
\toprule
arm & configuration & $n$ & mean & seed spread \\
\midrule
A1 & forward-only, no recycling & $3$ & $5.4541$ & $0.5295$ \\
A2 & bidirectional, no recycling & $2$ & $4.5436$ & $0.4075$ \\
A3 & bidirectional, tied loop & $3$ & $4.6011$ & $0.4084$ \\
A5 & forward-only, tied loop & $3$ & $5.4541$ & $0.5296$ \\
\bottomrule
\end{tabular}
\end{center}

Against the pre-registered bar --- a contrast counts only if it exceeds the larger of the two
arms' seed spreads --- the declared comparisons give:

\begin{center}
\begin{tabular}{llrrl}
\toprule
contrast & arms & difference & must clear & \\
\midrule
directionality & A2 $-$ A1 & $-0.9104$ & $0.5295$ & \textbf{separable} \\
the loop, bidirectional & A3 $-$ A2 & $+0.0575$ & $0.4084$ & not separable \\
the loop, forward-only & A5 $-$ A1 & $+0.0000$ & $0.5296$ & not separable \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Why the loop contrasts are not a result about recycling.} A5 reproduces A1 seed
by seed to four decimals: $5.6090$ against $5.6091$, $5.1118$ against $5.1118$, $5.6414$
against $5.6413$. A tie at that precision is not a small effect; it is the absence of one,
so we inspected the checkpoints rather than accepting the number. The recycling gate is a
scalar, initialised so that the loop is an \emph{identity} until trained, and after $1{,}908$
steps it sits at \textbf{$0.003039$} in A5 and \textbf{$0.000442$} in A3 --- having drifted
from $0.002973$ and $0.000404$ at step $1{,}500$. The loop's output is scaled by roughly
$0.3\%$. A1's checkpoint carries no \texttt{loop.*} keys at all, confirming that the arm
difference is what its name says.

So \emph{neither loop contrast is evidence about depth recycling}. They are measurements of
a mechanism that never engaged, and reporting them as ``recycling does not help'' would
attribute a null to the architecture when what was measured is an inert gate. This is the
``extra compute or a warm-start effect rather than demonstrated architecture advantage''
failure the protocol names in advance, arriving from a direction the protocol did not
anticipate: not a gain that fails to survive a matched comparison, but a mechanism that
never produced a gain to compare.

\paragraph{What the ablation does establish.} One lever moved, and by a wide margin.
\textbf{Directionality is separable at matched budget and parameter count}: A2 beats A1 by
$0.9104$ against a required $0.5295$, roughly $1.7$ times the bar. Bidirectional scanning
inside the denoising canvas is a real architectural effect on this object, measured, and it
is this paper's first confirmatory result. It is also the lever that costs nothing --- no
parameters, no cache, no extra forward passes.

The claim we therefore make, and the one we do not: \emph{direction helps; the recycled
depth, as initialised and configured here, did not engage within $2 \times 10^9$ tokens on a
$0.4$B backbone}. Whether it would at larger scale, on a longer budget, or under a different
gate initialisation is a question this measurement does not answer, and we do not answer it
by implication. A run whose gate opened would be a different experiment and would need its
own budget.

One caveat on precision: A2 is $n = 2$, because its seed-$17$ run log is absent although its
checkpoint reached step $1{,}908$. The directionality contrast is separable at $n = 2$
against the bar, but it is reported as $n = 2$ rather than as three.
...
\paragraph{An agreement can be an artifact of the test point.} A companion rule, learned
the same way: before writing an equivalence test, ask at which parameter values the two
implementations are \emph{forced} to agree, and refuse to test there. A gate that
asserted per-item equality between a batched generator and its one-at-a-time
predecessor \emph{at temperature zero} is the worked example --- at zero the sampler is
an argmax over the last axis, which is row-independent for any model whatsoever, so the
gate passes on a port with its padding mis-plumbed and its random stream shared. The
author had chosen the one setting where the two paths cannot differ. Measured at the
temperature every recorded number actually uses, naive batching reproduces the
single-item path for the first row and no other, which is precisely the pattern that
survives both checks a person would run: the equivalence test, blind by construction,
and a spot check of the first item. Our own suite had the same shape --- every
cross-check between our coordinate map and its inverse was \emph{relational}, so a
rotation applied consistently to both left the round-trip, the injectivity and the cell
count all green while every cell named the wrong task. The mapping is now pinned
absolutely, at named units, because an expectation computed by the implementation shares
its bugs. Both rules are instances of one habit: \emph{check the scope of the claim, not
only its arithmetic.}
...
\paragraph{A verdict can be right about what to do and wrong about what it knows.} The
third failure we record is the quietest, because it changes nothing the gate
\emph{decides} and only what it \emph{claims}. A boolean summarised three states ---
separable, not separable, and \emph{untestable} --- and on an input where the test
could not run at all it printed a formatted not-a-number and reported the score as
\emph{demonstrated} to be an artifact, from no measurement. Every assertion about the
decision was green throughout: untestable and tested-and-failed must both withhold
confirmation, so the behaviour was correct in every state and only the stated
justification was false. That is why it survived two corrections to the same gate that
did change the decision. The general form is worth stating because it bounds what
testing can catch: \emph{a failure of this kind is invisible to outcome-based tests by
construction}, so at least one test per gate must read the \emph{reason} a verdict was
reached and not merely the verdict. Our own evidence checker had the same defect and
we found it by applying this rule to it rather than by inspecting it: run without a
manuscript it reported a clean, complete result --- having never run coverage at all.
A second instance followed the same way, and it generalises the point: \emph{a skip is
a claim}. Every filter in an analysis path should be asked what its output would look
like if it skipped everything, and if the answer is \emph{a confident finding}, the
skip is unreported and the filter is the defect. Ours skipped numbers below a
magnitude threshold and ignored citation patterns; with the extraction pattern broken
so that it matched nothing at all, every downstream count was zero, nothing was
uncatalogued, and the verdict read \emph{pass}. The count is now always reported beside
its denominator, and a number extraction that yields nothing from a non-empty
manuscript withholds the verdict and names the extractor as the suspect rather than
sending the reader to inspect a manuscript that is fine.
...
verdict is therefore built from \emph{two axes} rather than enumerated: whether the
check \emph{ran}, and if it ran, what it \emph{found}. That gives three states ---
pass, finding, and \emph{withheld} --- and every reason the check could not run is data
attached to the third rather than a state of its own. Enumerating them is the tempting
form and it is the one that grows an exit per failure discovered, duplicating the axis
while losing it. A withheld verdict must carry both a reason and a \emph{remedy},
because a withhold with no route out is a block with better manners: the reason says
why the check could not run and the remedy says who can make it run. The same
discipline applies to the other half of the product, and we only saw it after asking
the question of a \emph{finding}: a failure has more than one cause, and they need
different responses --- a value absent from an artifact that exists means the claim or
the artifact is wrong, while an artifact the repository does not ship means the paper
depends on something missing. Both were reported as a bare \emph{finding}, so a reader
had to open the per-claim list to learn which --- and the third cause is the
likely one: a claim's context pattern can be narrower than the artifact's own
formatting, which produces exactly the same symptom as an absent value while the
artifact is entirely correct. Every such failure this checker actually met was
that one, and every one of them was initially diagnosed as the other. A finding
now says which of the three it is, and says which suspect to look at: the
pattern, or the artifact. The test that matters is therefore not
whether the check blocked but whether a reader can tell \emph{which} cause from the
output alone, and a finding now names its cause per claim.
