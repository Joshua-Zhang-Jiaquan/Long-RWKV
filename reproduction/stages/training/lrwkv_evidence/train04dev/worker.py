...
def main():
    ...
    ap.add_argument('--stop-step',type=int,choices=[300,700,1500],default=300);ap.add_argument('--resume',type=Path)
    ...
    source_files=[Path(__file__),Path(C.__file__),Path(W.__file__),Path(tasks.__file__),*sorted((args.model_root/'longrwkv').glob('*.py'))]
    contract=dict(version=2,seed=17,initial_seed=0,objective=args.objective,global_batch=32,per_rank_batch=4,
    ...
    for step in range(start_step+1,args.stop_step+1):
        ...
        canvases=[C.make_canvas(e,tok,17) for e in examples];batch=C.pack(canvases,device)
        optimizer.zero_grad(set_to_none=True);value=C.loss(ddp(batch),batch,args.objective)
        ...
        optimizer.step()
        values=torch.tensor([float(value),int(batch['masked'].sum()),int((batch['masked'].sum(-1)==0).sum())],device=device,dtype=torch.float64);dist.all_reduce(values)
        if rank==0:
            record=dict(step=step,n=n,mean_loss=float(values[0]/8),masked_targets=int(values[1]),empty_masks=int(values[2]),lr=rate,grad_norm_rank0=float(norm),elapsed_seconds=time.perf_counter()-started)
