#!/bin/bash
set -euo pipefail
: "${DISTANCE_ROOT:?}" "${DISTANCE_OUT:?}" "${DISTANCE_CHECKPOINT:?}"
cd "$DISTANCE_ROOT"
...
torchrun --standalone --nproc_per_node=8 -m lrwkv_evidence.distance_intervention.evaluate \
 --base /inspire/hdd/global_user/zhangjiaquan-253108540222/models/rwkv7-0.4B \
 --model-root "$DISTANCE_ROOT/model_source" --checkpoint "$DISTANCE_CHECKPOINT" \
 --out "$DISTANCE_OUT" > "$DISTANCE_OUT/torchrun.log" 2>&1
