...
def main():
    roles=[f'{a}_seed{s}' for s in (20271011,20271012,20271013) for a in ('near','balanced')]+['original']
    for role in roles:
    ...
    for role in roles:
        run('qz/submit_history_response.py','--role',role,'--submit')
    while True:
        ...
        for role in roles:
            ...
            if not dest.exists():
                run('theory_mvp/history_response/analyze.py','--role',role)
            else:
        ...
        if len(finished)==7:
            run('theory_mvp/history_response/analyze.py')
            return
        time.sleep(30)
