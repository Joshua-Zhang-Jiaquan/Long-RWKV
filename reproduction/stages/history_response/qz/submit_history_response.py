...
def main():
    ...
    folder=ROOT/'results/history_response';prior=ROOT/'results/distance_intervention';manifestpath=ROOT/'theory_mvp/history_response/FROZEN.json'
    ...
    sources['history_response_manifest.json']=manifestpath
    ...
    stage=Path(E.G)/'long_rwkv_history_response_stages'/digest
    out=Path(E.OUTPUTS)/f'history_response_{digest}_{args.role}'
    ...
    body=E.make_body(f'lrwkv-history-{digest[:8]}-{args.role}',E.wrapped(env,str(stage/'qz/launch_history_response.sh')),
                     'All16histories x four syndrome flips; fixed32problems x three16Kpositions; no retraining;8H100 under48cap',E.SPEC_8GPU)
    ...
    if args.submit:
        ...
        (stage/'history_response_sources.json').write_text(json.dumps(hashes,indent=2)+'\n')
