...
def main():
    ...
    if os.environ.get('TRITON_F32_DEFAULT')!='ieee':raise ValueError('IEEE required')
    ...
    paths=[Path(__file__),Path(R.__file__),Path(T.__file__),Path(W.__file__),Path(C.__file__),
           Path(O.__file__),Path(B.__file__),Path(M.__file__),Path(P.__file__),*sorted((args.model_root/'longrwkv').glob('*.py'))]
    if W.file_sha(args.initial_checkpoint)!=args.initial_sha256:raise ValueError('initial checkpoint checksum mismatch')
    recipe=dict(R.RECIPE,initialization='Selected seed71 independent2500; preserve labelled task; fresh context adaptation optimizer')
    ...
    stop=20 if args.qualification else 200
