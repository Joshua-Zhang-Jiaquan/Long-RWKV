...
from .exact_tasks import ExactTaskRequest, InfeasibleTaskError, generate_exact_task
from .models import (
    DATA_SEEDS,
    DISTRACTORS,
    FAMILIES,
    LENGTHS as CHARACTER_LENGTHS,
    LOADS,
    POSITIONS,
)
...
class TokenTargetedRequest:
    ...
    def as_character_request(self, declared_length: int = 4096) -> ExactTaskRequest:
        ...
        governs only the character framing, which this module discards -- but it
        is a PARAMETER rather than a constant, because the generator still
        enforces its own canvas feasibility before anything is discarded, and a
        high-load cell therefore has a minimum canvas it can be built on at all.
        See :func:`_generate_with_room`.
        """
        return ExactTaskRequest(
            family=self.family, data_seed=self.data_seed, declared_length=declared_length,
...
def _generate_with_room(request: TokenTargetedRequest):
    """Generate the evidence on the SMALLEST character canvas that can host it.

    ``declared_length`` is described as "only the framing, which this module
    discards", and for the framing that is true.  It is NOT true for feasibility:
    ``_positioned_prompt`` refuses when the evidence does not fit the canvas, and
    it does so before anything is discarded.  Measured on the bounded cut,
    load=128 carries 2,605 characters of evidence and is INFEASIBLE at the 4,096
    canvas this module used to hardcode, so the TOP OF THE LOAD AXIS -- the
    regime the state-capacity counterexample needs, K=V=64 against 128 pairs --
    could not be built at all.

    Escalating is safe because the evidence block is a function of
    (family, data_seed, load, distractor, instance_index) and NOT of
    declared_length, which only sets the ``x``/``y`` padding around it.  So the
    token fixture's evidence and gold are identical whichever canvas produced
    them; the canvas is an implementation detail of the generator, and this
    removes an accidental coupling between it and the load axis.
    """
    failures: list[str] = []
    for length in sorted(CHARACTER_LENGTHS):
        try:
            return generate_exact_task(request.as_character_request(length))
        except InfeasibleTaskError as exc:
            failures.append(f"{length}: {exc}")
    raise TokenTargetedError(
        f"{request.family} load={request.load} instance={request.instance_index} "
        f"does not fit ANY registered character canvas, so its evidence cannot be "
        f"generated by the registered generator: {failures}")
...
def generate_token_targeted_task(encode: Encode,
    ...
    source = _generate_with_room(request)
