...
def check_all(claims: list[Claim], root: Path) -> dict[str, object]:
    ...
    return {
        ...
        "note": ("a check that cannot run is reported as such rather than as a pass; "
                 "`external` is a disclosure, not a failure"),
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = check_all(load_inventory(args.inventory), args.root)
