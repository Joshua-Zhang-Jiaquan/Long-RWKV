...
cmd = spec['command']
for k, v in [("__CKPT__", ck), ("__TASKDIR__", f"{O}/tasks_m4loop_step{n}_raw_mmlu200"),
             ("__LOGNAME__", f"{O}/m4loop_probe_step{n}.log"), ("__MARK__", f"m4loop_probe step={n}"),
             ("__STEP__", str(n)), ("__RUNDIR__", rundir), ("__PROBELOG__", f"{O}/m4loop_probes.log")]:
    cmd = cmd.replace(k, v)
spec['command'] = cmd
...
PYEOF
  ...
  for D in $S/$RUN/step_*_probe_copy/; do
    ...
    n=$((10#$step))
    if ! grep -q "m4loop_probe step=$n exit=" "$O/m4loop_probes.log" 2>/dev/null; then continue; fi
    if [ $((n % 2000)) -eq 0 ]; then
      ...
      nm=$(ls -d /inspire/hdd/global_user/zhangjiaquan-253108540222/cap_lm_m4loop_s$n/*/merged/ 2>/dev/null | wc -l)
      [ "$nm" -ge 5 ] && rm -rf "$D"
    else
      rm -rf "$D"
    fi
