...
def main():
    ...
    original=json.loads((folder/'plan_ca0994b91d5c1222_qualification.json').read_text())
    ...
    if args.qualification:
        ...
        for p in (ROOT/'lrwkv_evidence/long_context_transfer').glob('*.py'):sources[str(p.relative_to(ROOT))]=p
        sources['qz/launch_long_context_transfer.sh']=ROOT/'qz/launch_long_context_transfer.sh'
    else:
    ...
    stage=Path(E.G)/'long_rwkv_long_context_transfer_stages'/digest
    phase='qualification' if args.qualification else 'development600'
    out=Path(E.OUTPUTS)/f'long_context_transfer_{digest}_{phase}'
    env=dict(LCM_ROOT=str(stage),LCM_OUT=str(out),LCM_QUALIFICATION=str(int(args.qualification)),
             LCT_INITIAL_CHECKPOINT=str(checkpoint),LCT_INITIAL_SHA256=done['checkpoint_sha256'])
    ...
    body=E.make_body(f'lrwkv-lct-{digest[:8]}-{phase}',E.wrapped(env,str(stage/'qz/launch_long_context_transfer.sh')),
                     'Controlled auxiliary-conditioner warm start; same600 task adaptation;8H100 under48cap',E.SPEC_8GPU)
