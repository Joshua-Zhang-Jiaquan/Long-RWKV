"""Submit all-cell exact evaluation of one fixed distance-study checkpoint."""
import argparse
import hashlib
import json
...
def main():
    ...
    folder=ROOT/'results/distance_intervention';manifestpath=ROOT/'theory_mvp/distance_intervention/FROZEN_EVALUATION.json'
    manifest=json.loads(manifestpath.read_text())
    roles=['original']+[f'{a}_seed{s}' for s in (20271011,20271012,20271013) for a in ('near','balanced')]
    ...
    trainpath=ROOT/'results/train04confirm/plan_8cd809d08a96ea13_confirm_independent_seed71.json' if args.role=='original' else folder/f'plan_{args.role}.json'
    ...
    sources['distance_eval_manifest.json']=manifestpath
    ...
    stage=Path(E.G)/'long_rwkv_distance_evaluation_stages'/digest
    out=Path(E.OUTPUTS)/f'distance_evaluation_{digest}_{args.role}'
    ...
    cache_seed=None
    cache_receipt=folder/'evaluation_cache_seed.json'
    if args.role!='original' and cache_receipt.exists():
        ...
        if original['manifest_sha256']!=sha(manifestpath):raise ValueError('cache seed is from a different evaluation runtime')
        if not Path(cache_seed['cache']).is_dir():raise ValueError('shared compiled cache missing')
        env['TRITON_CACHE_DIR']=cache_seed['cache']
    body=E.make_body(f'lrwkv-dist-eval-{digest[:8]}-{args.role}',E.wrapped(env,str(stage/'qz/launch_distance_evaluation.sh')),
                     'Frozen exact16K distance and evidence-response panel; all144conditions;8H100 under48cap',E.SPEC_8GPU)
    ...
    record=dict(stage=str(stage),out=str(out),sources=hashes,body=body,role=args.role,training_plan=str(trainpath),
                checkpoint_sha256=done['checkpoint_sha256'],manifest_sha256=sha(manifestpath))
    if cache_seed:record['compiled_cache_seed']=cache_seed
    dest=folder/f'plan_eval_{args.role}.json'
    ...
    if args.submit:
        ...
        (stage/'distance_eval_sources.json').write_text(json.dumps(hashes,indent=2)+'\n')
