...
args=(--base /inspire/hdd/global_user/zhangjiaquan-253108540222/models/rwkv7-0.4B --model-root "$LCM_ROOT/model_source" --out "$LCM_OUT")
if [[ "$LCM_QUALIFICATION" == 1 ]]; then args+=(--qualification); else args+=(--qualification-out "${LCM_QUALIFICATION_OUT:?}"); fi
exec torchrun --standalone --nproc_per_node=8 -m lrwkv_evidence.long_context_mvp.worker "${args[@]}" > "$LCM_OUT/torchrun.log" 2>&1
