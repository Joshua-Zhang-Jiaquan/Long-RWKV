...
def main():
    ...
    stage=Path(E.G)/'long_rwkv_long_context_eval_stages'/digest;out=Path(E.OUTPUTS)/f'long_context_eval_{digest}'
    env=dict(LCM_ROOT=str(stage),LCM_EVAL_OUT=str(out),LCM_CHECKPOINT=str(checkpoint),TRITON_F32_DEFAULT='ieee',TRITON_CACHE_DIR=str(out/'triton_ieee_cache'))
    body=E.make_body(f'lrwkv-lcm-eval-{digest[:8]}',E.wrapped(env,str(stage/'qz/launch_long_context_eval.sh')),'Development-only exact endpoint and uncached cost gate;8H100',E.SPEC_8GPU)
    ...
    dest=ROOT/'results/long_context_mvp'/f'plan_eval_{digest}.json'
