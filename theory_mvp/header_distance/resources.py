...
def main():
    folder=ROOT/'results/header_distance';rows=[];events=[]
    plans=sorted(folder.glob('plan_*.json'))+sorted((folder/'queued_attempts').glob('plan_*.json'))
    for path in plans:
        ...
        rows.append(dict(plan=str(path.relative_to(ROOT)),superseded_attempt=path.parent.name=='queued_attempts',role=p['role'],job_id=receipt['job_id'],status=status,project_id=project,
    ...
    if sum(not r['superseded_attempt'] for r in rows)!=6:raise ValueError('wrong evaluation set')
    cancelled=[r for r in rows if r['superseded_attempt']]
    if len(cancelled)!=1 or cancelled[0]['status']!='job_stopped':raise ValueError('unexpected superseded attempt')
    ...
    result=dict(observed_at=C.timestamp(),all_jobs_terminal=True,jobs=rows,
                ...
                scope='Six completed evaluations plus one stopped startup-race attempt; its scheduler reports zero runtime although startup logs exist. Excludes previous study and prior checkpoint training. Intervals include queued reservation. Utilization covers the entire observed job, including loading and numerical screens.')
