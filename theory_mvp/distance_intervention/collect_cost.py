...
def collect(planpath):
 ...
 for rank in range(8):
  ...
  if len(rows)!=6 or [r['kind'] for r in rows]!=['provenance','numerical_screen']+['condition']*3+['complete']:raise ValueError('incomplete cost shard')
  p=rows[0]
  if p['rank']!=rank or p['world']!=8 or p['comparator']!=kind or p['checkpoint_sha256']!=plan['checkpoint_sha256'] or p['design_sha256']!=plan['design_sha256'] or p['final_panel_sha256']!=design['final_design_sha256'] or p['budget_seconds']!=1.5 or p['timing_repeats']!=5:raise ValueError('wrong provenance')
  validate_numerical(rows[1]['rows']);expected=[(i,pos) for i in (rank,) for pos in ('far','middle','near')]
  ...
  for r in rows[2:-1]:
   ...
   for c in r['costs']:
    if c['calls']!=(1 if c['method']=='one' else 2) or len(c['seconds'])!=5 or len(c['sample_outputs'])!=5 or not all(math.isfinite(t) and t>0 for t in c['seconds']) or any(type(v) is not int or not 0<=v<256 for v in c['sample_outputs']) or c['peak_allocated_bytes']<=0:raise ValueError('invalid timing record')
   conditions.append(r)
 summaries=[]
 for method in design['methods']:
  ...
  summaries.append(dict(method=method,mean_request_seconds=statistics.mean(perbase),base_mean_seconds=perbase,peak_allocated_bytes=max(peaks),**bootstrap_mean(perbase,seed=20271022,repeats=10000)))
