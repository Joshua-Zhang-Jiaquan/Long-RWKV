"""Reporting-only repair: preserve frozen endpoints while normalizing JSON arrays for equality."""
...
def summarize(rows,panel):
    ...
    for record,ex in panel:
        for method in METHODS:
            ...
            if row['n']!=8 or row['family']!=record['family'] or row['instance_id']!=record['instance_id'] or row['structure']!=record['structure'] or tuple(map(tuple,row['valid_endpoints']))!=tuple(map(tuple,support(ex))):
...
def collect(out,stage):
    from lrwkv_evidence.train04confirm.evaluate import load_panel
