...
from lrwkv_evidence.posterior_context_probe import tasks as T
...
def collect(planpath):
    ...
    conditions=[];gates=[];rawhash={}
    for rank in range(8):
        path=out/f'rank{rank}.jsonl';rawhash[str(path)]=file_sha(path)
        rows=[json.loads(line) for line in path.read_text().splitlines()];ex=T.example(rank)
        if not rows or rows[0]['kind']!='provenance' or rows[-1]['kind']!='complete':raise ValueError('incomplete rank')
        p=rows[0]
        if p['rank']!=rank or p['world']!=8 or p['seed']!=T.SEED or p['split']!='dev' or p['checkpoint_sha256']!=plan['checkpoint_sha256']:raise ValueError('wrong provenance')
        gate=[r for r in rows if r['kind']=='gate']
        if len(gate)!=1:raise ValueError('missing gate')
        gate=gate[0];gates.append(gate)
        expected=[(None,'evidence_only')]+([(n,pos) for n in (1024,4096,16384) for pos in ('far','middle','near')] if gate['all_passed'] else [])
        ...
        if len(screens)!=int(gate['all_passed']):raise ValueError('bad numerical screen count')
        if screens:validate_numerical(screens[0]['rows'])
        if len(rows)!=len(observed)+3+len(screens) or rows[-1]['long_sweep_complete']!=gate['all_passed']:raise ValueError('unexpected raw rows')
        ...
        for row in observed:
            ...
            if ser!={k:v for k,v in fresh.items() if k!='ids'} or row['index']!=rank or row['family']!=ex['family']:raise ValueError('wrong serialized condition')
            exact=row['exact']
        ...
        info=next(r for r in observed[0]['exact']['results'] if r['method']=='information_set')
        if gate['local_passed']!=(info['exact_valid_mass']>=.9 and info['joint_kl_nats']<=.2):raise ValueError('bad local gate')
    passed=all(g['local_passed'] for g in gates)
    if any(g['all_passed']!=passed for g in gates):raise ValueError('bad global gate')
    summary=[]
    ...
    return dict(execution_complete=True,short_gate_passed=passed,long_sweep_complete=passed,checkpoint_sha256=plan['checkpoint_sha256'],plan=str(planpath.resolve()),raw_sha256=rawhash,summary=summary,conditions=conditions,scope='Selected checkpoint and fresh development conditions; descriptive evidence only')
...
def main():
    ...
    lines=['# Label-preserving development probe','',result['scope'],'',f"Short gate: {result['short_gate_passed']}; long sweep complete: {result['long_sweep_complete']}",'','| Family | Context | Position | Policy | Mean KL | Mean valid mass |','|---|---:|---|---|---:|---:|']
