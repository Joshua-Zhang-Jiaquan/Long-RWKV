...
def analyze(conditions, indices, *, bootstrap_seed=20270925, repeats=10000):
    ...
    expected = {(family, index, length, position)
                for family in ('retrieval', 'dependent') for index in indices
    ...
    for family in ('retrieval', 'dependent'):
        for comparator in CONTRASTS:
            for metric in METRICS:
                short = differences(family, None, 'middle', comparator, metric)
                for length, position in CONTEXTS:
    ...
    for comparator in CONTRASTS:
        for metric in METRICS:
            by_problem = np.stack([differences('dependent', 16384, pos, comparator, metric)
                                   for pos in ('far', 'middle', 'near')]).mean(axis=0)
            primary.append(dict(family='dependent', context_tokens=16384,
