#!/usr/bin/env bash
# Direct qz submission for the calibration-v13 payload: forward-only per-rung warm-up, a cache
# release between rungs, and expandable_segments to survive dplr-chunk fragmentation. (Task-6 throughput forecast input).
...
readonly V13=${ROOT}/payloads/qualification-20260915-13-global-ffaa464d-calibration-v13
readonly EV=.omo/evidence/nonlatent-rnn-dlm-iclr-research/qualification-20260915-13-live
readonly MANIFEST_SHA=7a442fb328b02165c450382b390d505c8d7a63e66bd616bcf651578958972e8f
...
python3 - "$SPEC" "$RUN_ID" "$NONCE" "$MANIFEST_SHA" "$RECEIPT" "$V13" "$IMAGE" <<'PY'
import json, sys
spec, run_id, nonce, manifest_sha, receipt, v13, image = sys.argv[1:]
body = {
    "auto_fault_tolerance": False,
    "command": f"/usr/bin/bash {v13}/scale/experiments/nonlatent_iclr/qualification/run_qualification.sh",
