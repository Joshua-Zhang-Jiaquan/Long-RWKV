#!/usr/bin/env bash
# Direct qz submission for the calibration-v14 payload: best-effort warm-up inside the OOM guard,
# a cache release between rungs, and expandable_segments for dplr-chunk fragmentation. (Task-6 throughput forecast input).
...
readonly V14=${ROOT}/payloads/qualification-20260915-14-global-ffaa464d-calibration-v14
readonly EV=.omo/evidence/nonlatent-rnn-dlm-iclr-research/qualification-20260915-14-live
readonly MANIFEST_SHA=147e6d7f007c09f261523069b2e34d2b6732bd724d500bbdc2ee133ca96dc311
...
python3 - "$SPEC" "$RUN_ID" "$NONCE" "$MANIFEST_SHA" "$RECEIPT" "$V14" "$IMAGE" <<'PY'
import json, sys
spec, run_id, nonce, manifest_sha, receipt, v14, image = sys.argv[1:]
body = {
    "auto_fault_tolerance": False,
    "command": f"/usr/bin/bash {v14}/scale/experiments/nonlatent_iclr/qualification/run_qualification.sh",
