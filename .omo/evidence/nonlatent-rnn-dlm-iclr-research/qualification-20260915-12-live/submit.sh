#!/usr/bin/env bash
# Direct qz submission for the calibration-v12 payload: forward depth override, per-rung
# untimed warm-ups, device-bound synchronize, reporting aggregation. (Task-6 throughput forecast input).
...
readonly V12=${ROOT}/payloads/qualification-20260915-12-global-ffaa464d-calibration-v12
readonly EV=.omo/evidence/nonlatent-rnn-dlm-iclr-research/qualification-20260915-12-live
readonly MANIFEST_SHA=30b20b7d5d82667638a874c05a7149b106ceccd726abb5b2c9bbe34e733c7bf4
...
python3 - "$SPEC" "$RUN_ID" "$NONCE" "$MANIFEST_SHA" "$RECEIPT" "$V12" "$IMAGE" <<'PY'
import json, sys
spec, run_id, nonce, manifest_sha, receipt, v12, image = sys.argv[1:]
body = {
    "auto_fault_tolerance": False,
    "command": f"/usr/bin/bash {v12}/scale/experiments/nonlatent_iclr/qualification/run_qualification.sh",
