#!/usr/bin/env bash
# Direct qz submission for the lifecycle-v7 qualification payload.
# Generates fresh run identity, submits exactly one CreateJob, and writes the
# binding receipt the worker launcher awaits. No retries.
set -Eeuo pipefail

readonly ROOT=/inspire/hdd/global_user/zhangjiaquan-253108540222/nonlatent_iclr_qualification
readonly V7=${ROOT}/payloads/qualification-20260914-08-global-ffaa464d-semantics-v8
readonly EV=.omo/evidence/nonlatent-rnn-dlm-iclr-research/qualification-20260914-08-live
readonly MANIFEST_SHA=bae0fa5a71aca185d1718757d56de2d8f01a2606575d2d907b7a0d88e82ab68a
...
python3 - "$SPEC" "$RUN_ID" "$NONCE" "$MANIFEST_SHA" "$RECEIPT" "$V7" "$IMAGE" <<'PY'
import json, sys
spec, run_id, nonce, manifest_sha, receipt, v7, image = sys.argv[1:]
body = {
    "auto_fault_tolerance": False,
    "command": f"/usr/bin/bash {v7}/scale/experiments/nonlatent_iclr/qualification/run_qualification.sh",
    "description": "Direct-qz nonlatent lifecycle qualification: 1x8 H100 SXM 80GB, 30 minutes, zero retries.",
