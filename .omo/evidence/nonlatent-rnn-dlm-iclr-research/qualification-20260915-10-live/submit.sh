#!/usr/bin/env bash
# Direct qz submission for the calibration-v10 payload (corrected staged-root derivation). (Task-6 throughput forecast input).
...
readonly V10=${ROOT}/payloads/qualification-20260915-10-global-ffaa464d-calibration-v10
readonly EV=.omo/evidence/nonlatent-rnn-dlm-iclr-research/qualification-20260915-10-live
readonly MANIFEST_SHA=fcbd2a9195f1b4f8a97df16c1df3d564f294edd9b5b6354841169d03fd9891c1
...
python3 - "$SPEC" "$RUN_ID" "$NONCE" "$MANIFEST_SHA" "$RECEIPT" "$V10" "$IMAGE" <<'PY'
import json, sys
spec, run_id, nonce, manifest_sha, receipt, v10, image = sys.argv[1:]
body = {
    "auto_fault_tolerance": False,
    "command": f"/usr/bin/bash {v10}/scale/experiments/nonlatent_iclr/qualification/run_qualification.sh",
