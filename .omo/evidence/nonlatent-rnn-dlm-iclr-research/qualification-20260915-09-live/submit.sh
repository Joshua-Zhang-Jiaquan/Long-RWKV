#!/usr/bin/env bash
# Direct qz submission for the calibration-v9 payload (Task-6 throughput forecast input).
...
readonly V9=${ROOT}/payloads/qualification-20260915-09-global-ffaa464d-calibration-v9
readonly EV=.omo/evidence/nonlatent-rnn-dlm-iclr-research/qualification-20260915-09-live
readonly MANIFEST_SHA=71f7f43250083dcdd06d65cbde71aaf87ec0d8343e1f8b051b79365b1e9882c5
...
python3 - "$SPEC" "$RUN_ID" "$NONCE" "$MANIFEST_SHA" "$RECEIPT" "$V9" "$IMAGE" <<'PY'
import json, sys
spec, run_id, nonce, manifest_sha, receipt, v9, image = sys.argv[1:]
body = {
    "auto_fault_tolerance": False,
    "command": f"/usr/bin/bash {v9}/scale/experiments/nonlatent_iclr/qualification/run_qualification.sh",
