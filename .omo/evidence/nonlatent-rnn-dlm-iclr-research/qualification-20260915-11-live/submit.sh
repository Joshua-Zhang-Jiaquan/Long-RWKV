#!/usr/bin/env bash
# Direct qz submission for the calibration-v11 payload: device-explicit CUDA accounting,
# per-rank stderr capture, and arm-failure isolation. (Task-6 throughput forecast input).
...
readonly V11=${ROOT}/payloads/qualification-20260915-11-global-ffaa464d-calibration-v11
readonly EV=.omo/evidence/nonlatent-rnn-dlm-iclr-research/qualification-20260915-11-live
readonly MANIFEST_SHA=73e30af16665fd48ff3465583626b7601c6a4e6cd29bf1ae24c6664e0650c23f
...
python3 - "$SPEC" "$RUN_ID" "$NONCE" "$MANIFEST_SHA" "$RECEIPT" "$V11" "$IMAGE" <<'PY'
import json, sys
spec, run_id, nonce, manifest_sha, receipt, v11, image = sys.argv[1:]
body = {
    "auto_fault_tolerance": False,
    "command": f"/usr/bin/bash {v11}/scale/experiments/nonlatent_iclr/qualification/run_qualification.sh",
