...
def main():
    ...
    for rel,digest in json.loads((root/'adapt_eval_sources.json').read_text()).items():
    ...
    if payload['step']!=200 or contract['recipe']['terminal_step']!=200 or W.canonical_sha(contract)!=payload['contract_sha256'] or contract['initial_checkpoint_sha256']!='52dbc7f289513e475d10422dbccffa52f74ec55fc887253ea13ae0476dc3e2c9':raise ValueError('bad adapted checkpoint lineage')
    for rel,digest in contract['source_hashes'].items():
        if W.file_sha(root/rel)!=digest:raise ValueError('changed training source')
    ...
    with (args.out/f'rank{rank}.jsonl').open('x') as stream:
        ...
        emit(dict(kind='provenance',rank=rank,world=8,checkpoint_sha256=checkpoint_sha,manifest_sha256=manifest_sha,seed=T.SEED,split='dev',runtime=metadata(torch,torch.device('cuda',local)),selection='seed71 independent2500 plus fixed200 context adaptation; development only',serialization='original public output labels beside values; original prompt token sequence intact at evidence-only',scope='No final confirmation or broad quality claim'))
        ex=T.example(rank)
