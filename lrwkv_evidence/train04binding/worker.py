"""Candidate public-label serialization intervention from matched phase300 parents."""
import argparse
import json
import os
...
def qualify(model,packed,tok,device):
    model.eval();checks=[]
    for n in (2,4,8):
        examples=[C.example('train',17,i,n) for i in range(4)]
        canvases=[B.decorate(ex,C.make_canvas(ex,tok,17,stage=4),tok) for ex in examples]
        batch=B.pack(canvases,device);together=packed(batch)
        separate=torch.cat([packed(B.pack([c],device)) for c in canvases])
...
def development(packed,tok,rank,device,n):
    ...
    for index in range(rank,64,8):
        ...
        for stage in range(1,9):canvases.append(B.decorate(ex,C.make_canvas(ex,tok,90210,stage=stage),tok))
    for start in range(0,len(canvases),4):
        batch=B.pack(canvases[start:start+4],device);lp=packed(batch).double().log_softmax(-1);p=batch['oracle'].double();mask=batch['masked']
...
def main():
    ...
    ap.add_argument('--stop-step',type=int,choices=[320,700,1500],default=700);ap.add_argument('--resume',type=Path);ap.add_argument('--parent',type=Path,required=True)
    ...
    ap.add_argument('--mask-law',choices=['ordinary','mixture'],required=True)
    ...
    source_files=[Path(__file__),Path(C.__file__),Path(M.__file__),Path(B.__file__),Path(E.__file__),Path(P.__file__),Path(W.__file__),Path(tasks.__file__),*sorted((args.model_root/'longrwkv').glob('*.py'))]
    ...
    contract=dict(version=4,serialization='public output labels beside value slots v1',seed=17,initial_seed=0,objective=args.objective,mask_law=args.mask_law,parent_checkpoint_sha256=parent_sha,global_batch=32,per_rank_batch=4,
                  dtype='FP32 tensors; PyTorch TF32 off; Triton backend default retained; deterministic algorithms requested',head='FP32 binary log-odds of original head rows',execution='separate document forwards',
    ...
    for step in range(start_step+1,args.stop_step+1):
        ...
        canvases=[B.decorate(e,M.canvas(e,tok,17) if args.mask_law=='mixture' else C.make_canvas(e,tok,17),tok) for e in examples]
        batch=B.pack(canvases,device)
        optimizer.zero_grad(set_to_none=True);value=(M.loss if args.mask_law=='mixture' else C.loss)(ddp(batch),batch,args.objective)
        ...
        optimizer.step()
        entropy=torch.where(batch['oracle']==.5,torch.log(torch.tensor(2.,device=device)),0.)
        entropy_baseline=M.oracle_entropy(batch) if args.mask_law=='mixture' else ((entropy*batch['masked']).sum(-1)*8/(batch['stage']*n)).mean()
        policy_count=int(batch['policy_branch'].sum()) if args.mask_law=='mixture' else 0
        values=torch.tensor([float(value),int(batch['masked'].sum()),int((batch['masked'].sum(-1)==0).sum()),float(entropy_baseline),policy_count,int(batch['input_ids'].numel())],device=device,dtype=torch.float64);dist.all_reduce(values)
        if rank==0:
            record=dict(step=step,n=n,mean_loss=float(values[0]/8),masked_targets=int(values[1]),empty_masks=int(values[2]),lr=rate,grad_norm_rank0=float(norm),oracle_entropy=float(values[3]/8),policy_examples=int(values[4]),global_input_tokens=int(values[5]),elapsed_seconds=time.perf_counter()-started)
