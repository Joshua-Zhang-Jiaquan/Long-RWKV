...
METHODS=('one','information_set','random_halves','pair_preserving_halves','sequential')


def partitions(ex):
    order=list(range(8));T.rng('reveal',ex['instance_id']).shuffle(order)
    return dict(one=[list(range(8))],information_set=[list(range(4)),list(range(4,8))],
                random_halves=[order[:4],order[4:]],pair_preserving_halves=[[0,1,4,5],[2,3,6,7]],
                sequential=[[i] for i in range(8)])
