"""Independent header/task placement; exact one-call and two-call endpoint laws."""
import argparse
import json
import os
...
def main():
    ...
    for rel,digest in json.loads((root/'header_distance_sources.json').read_text()).items():
        if W.file_sha(root/rel)!=digest: raise ValueError('frozen evaluation source mismatch: '+rel)
    manifest=json.loads((root/'header_distance_manifest.json').read_text())
    ...
    with (args.out/f'rank{rank}.jsonl').open('x') as stream:
        ...
        emit(dict(kind='provenance',rank=rank,world=8,role=role,checkpoint_sha256=checkpoint_sha,
                  manifest_sha256=W.file_sha(root/'header_distance_manifest.json'),seed=T.SEED,followup='header_distance_v1',
                  runtime=metadata(torch,torch.device('cuda',local)),identity=identity,
                  selection='fresh32problems; six fixed adaptation checkpoints; selected seed71 lineage'))
        check=T.example(0)
        def screen(spec):
            visible={} if spec['history']=='all_masked' else {i:0 for i in check['information_set']}
            return predict(check,T.legacy_serialize(check,tok,spec['context_tokens'],spec['position']),visible)
        emit(dict(kind='numerical_screen',rows=numerical_screen(screen)))
        screens=[]
        for cell in T.CELLS[:4]:
            encoded=T.serialize(check,tok,cell)
            for history in ('all_masked','information_set_zero'):
                visible={} if history=='all_masked' else {i:0 for i in check['information_set']}
                a=predict(check,encoded,visible);b=predict(check,encoded,visible)
                ...
                if not torch.isfinite(a).all() or within>1e-5 or cross>1e-4:raise ValueError('factorial numerical instability')
                screens.append(dict(cell=cell,history=history,within=within,cross=cross))
        emit(dict(kind='numerical_factorial',rows=screens))
        count=0
        for index in range(rank,32,8):
            ex=T.example(index)
            for cell in T.CELLS:
                encoded=T.serialize(ex,tok,cell);started=time.perf_counter()
                initial=predict(ex,encoded,{}).cpu().tolist()
                conditional=[predict(ex,encoded,visible).cpu().tolist() for visible in T.histories(ex)]
                emit(dict(kind='condition',index=index,cell=cell,
                          serial={k:v for k,v in encoded.items() if k!='ids'},
                          initial=initial,conditional=conditional,
                          metrics=T.endpoint(ex,initial,conditional),audit_seconds=time.perf_counter()-started))
                count+=1
