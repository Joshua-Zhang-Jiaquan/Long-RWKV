...
def main():
 ...
 ap.add_argument('--kind',choices=('attention',),required=True);args=ap.parse_args()
 ...
 with (args.out/f'rank{rank}.jsonl').open('x') as stream:
  ...
  emit(dict(kind='provenance',comparator=args.kind,rank=rank,world=8,checkpoint_sha256=sha,checkpoint_step=200 if args.kind=='rwkv' else 20,design_sha256=W.file_sha(root/'cost_design.json'),final_panel_sha256=W.file_sha(root/'final_design.json'),runtime=metadata(torch,torch.device('cuda',local)),precision='FP32/IEEE;TF32 disabled',timing_repeats=5,budget_seconds=1.5,scope='Mean complete-request latency at batch1; no per-request deadline guarantee; attention quality is not measured'))
  check=T.example(1)
  def screen(spec):
  ...
  for index in (rank,):
   ...
   for position in ('far','middle','near'):
    ...
    for method,groups in T.partitions(ex).items():
     request(groups,20271023+index*100);torch.cuda.synchronize();times=[];peak=0;outputs=[]
     for rep in range(5):
      torch.cuda.reset_peak_memory_stats();torch.cuda.synchronize();start=time.perf_counter();y=request(groups,20271024+index*100+rep);torch.cuda.synchronize();times.append(time.perf_counter()-start);outputs.append(y);peak=max(peak,torch.cuda.max_memory_allocated())
